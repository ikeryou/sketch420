...
export class Item extends MyDisplay {
  ...
  protected _update():void {
    ...
    const rad = this._c * -3 + this._itemId * 2
    const p1 = 0;
    const p2 = 50 + (Math.sin(Util.radian(rad)) * 25);
    const p3 = 100;
    ...
    this._color3.r = Util.map(Math.sin(Util.radian(this._angle1)), 0, 1, -1, 1)
    ...
    const angle = Math.cos(Util.radian(rad)) * 80
